...
void LteUeRrc::setSwitchedToState(){
  ...
  sem->SetCurrentA (0.006);
  // batteryPowerMeasure = PrintCellInfo (es);
  ...
  void LteUeRrc::ValidateResource(){

    Ptr<LteUePhy> lteUePhy = CreateObject<LteUePhy>();
    double txPower =lteUePhy->GetTxPower();
    State currentState = lteUeRrc->getState();
    if (currentState(string) == "IDLE_RANDOM_ACCESS" ){
        if (txPower >= lteUePhy->GetTxPower()){
          counter++;
    ...
    if(counter == 6) {
      ...
      lteUeRrc->DoDispose();
      std::cout<<"counter ="<<counter<<","<<"initiate back off";
    }
     start = std::clock();
     lteUeRrc = CreateObject<LteUrRrc>;
     lteUeRrc->DoInitialize();

    if (threshold >= 10 && duration <= 30 ){
      std::cout<<"abnormal event detected initiates back off";
      lteUeRrc->DoDisconnect();
      lteUeRrc->DoDispose();
      threshold =0;
